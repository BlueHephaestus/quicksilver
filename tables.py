...
def get_threshold_tables(session):
    ...
    str_grid_ns = np.zeros_like(grid_ns, dtype=np.object)
    str_grid_ps = np.zeros_like(grid_ps, dtype=np.object)
    for i in range(4):
        for j in range(4):
            if i == 3 or j == 3:
                # Bold totals
                str_grid_ns[i][j] = f" <br><b>{grid_ns[i][j]}</b>"
                str_grid_ps[i][j] = f" <br><b>{grid_ps[i][j]:.2f}</b>"
            else:
                str_grid_ns[i][j] = f" <br>{grid_ns[i][j]}"
                str_grid_ps[i][j] = f" <br>{grid_ps[i][j]:.2f}"
    ...
    str_grid_ns = np.transpose(str_grid_ns)
    str_grid_ps = np.transpose(str_grid_ps)
    fig_ns = go.Figure(data=[go.Table(
        header=None,
        cells=dict(values=str_grid_ns, height=80),
    )])
    fig_ps = go.Figure(data=[go.Table(
        header=None,
        cells=dict(values=str_grid_ps, height=80, suffix="%"),
    )])
    ...
    return fig_ns, fig_ps, grid_ns, grid_ps
